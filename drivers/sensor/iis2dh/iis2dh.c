...
#if defined(DT_ST_IIS2DH_BUS_SPI)
#include <drivers/spi.h>
#elif defined(DT_ST_IIS2DH_BUS_I2C)
...
/* gains in uG/LSB */
static const u32_t iis2dh_gain[3][4] = {
	{
		/* HR mode */
		980/16,		/* 2G */
		1950/16,	/* 4G */
...
static int iis2dh_set_fs_raw(struct iis2dh_data *iis2dh, u8_t fs)
{
	int err;
 ...
 */
static int iis2dh_set_range(struct device *dev, u16_t range)
{
	int err;
	struct iis2dh_data *iis2dh = dev->driver_data;
	u8_t fs = IIS2DH_FS_TO_REG(range);
 ...
 */
static int iis2dh_set_odr(struct device *dev, u16_t odr)
{
	struct iis2dh_data *iis2dh = dev->driver_data;
	const struct iis2dh_device_config *cfg = dev->config->config_info;
	iis2dh_odr_t val;
...
const struct iis2dh_device_config iis2dh_cfg = {
	.bus_name = DT_INST_0_ST_IIS2DH_BUS_NAME,
	.pm = CONFIG_IIS2DH_POWER_MODE,
#ifdef CONFIG_IIS2DH_TRIGGER
	.int_gpio_port = DT_INST_0_ST_IIS2DH_DRDY_GPIOS_CONTROLLER,
	.int_gpio_pin = DT_INST_0_ST_IIS2DH_DRDY_GPIOS_PIN,
	.int_gpio_flags = DT_INST_0_ST_IIS2DH_DRDY_GPIOS_FLAGS,
#if defined(CONFIG_IIS2DH_INT_PIN_1)
	.int_pin = 1,
#elif defined(CONFIG_IIS2DH_INT_PIN_2)
	.int_pin = 2,
#endif /* CONFIG_IIS2DH_INT_PIN_* */
...
DEVICE_AND_API_INIT(iis2dh, DT_INST_0_ST_IIS2DH_LABEL, iis2dh_init,
