...
#if defined(CONFIG_LIS2DU12_ENABLE_TEMP)
/**
 * lis2du12_enable_t_int - TEMP enable selected int pin to generate interrupt
 */
static int lis2du12_enable_t_int(const struct device *dev, int enable)
{
	...
	if (enable) {
		...
		lis2du12_temperature_raw_get(ctx, &buf);
	}
	...
	return lis2du12_pin_int2_route_set(ctx, val);
}
#endif
			  ...
			  sensor_trigger_handler_t handler)
{
	...
	if (trig->chan == SENSOR_CHAN_ACCEL_XYZ) {
		...
		lis2du12->handler_drdy_temp = handler;
		lis2du12->trig_drdy_temp = trig;
		if (handler) {
			return lis2du12_enable_t_int(dev, LIS2DU12_EN_BIT);
		} else {
			return lis2du12_enable_t_int(dev, LIS2DU12_DIS_BIT);
		}
	}
#endif
...
int lis2du12_init_interrupt(const struct device *dev)
{
	...
	k_thread_create(&lis2du12->thread, lis2du12->thread_stack,
			...
			0, K_NO_WAIT);
	k_thread_name_set(&lis2du12->thread, "lis2du12");
#elif defined(CONFIG_LIS2DU12_TRIGGER_GLOBAL_THREAD)
