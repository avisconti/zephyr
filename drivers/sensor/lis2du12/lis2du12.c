...
static int lis2du12_sample_fetch_temp(const struct device *dev)
{
	const struct lis2du12_config *cfg = dev->config;
	stmdev_ctx_t *ctx = (stmdev_ctx_t *)&cfg->ctx;
	struct lis2du12_data *data = dev->data;

	if (lis2du12_temperature_raw_get(ctx, &data->temp_sample) < 0) {
		LOG_DBG("Failed to read sample");
		return -EIO;
	}
				...
				enum sensor_channel chan)
{
	...
	case SENSOR_CHAN_DIE_TEMP:
		lis2du12_sample_fetch_temp(dev);
		break;
#endif
...
#if defined(CONFIG_LIS2DU12_ENABLE_TEMP)
static void lis2du12_channel_get_temp(struct sensor_value *val,
				       struct lis2du12_data *data)
{
	int32_t micro_c;
...
#if defined(CONFIG_LIS2DU12_ENABLE_TEMP)
	case SENSOR_CHAN_DIE_TEMP:
		lis2du12_channel_get_temp(val, data);
		break;
#endif
	default:
		return -ENOTSUP;
...
static int lis2du12_init_chip(const struct device *dev)
{
	...
	if (chip_id.whoami != LIS2DU12_ID) {
		LOG_DBG("Invalid chip id 0x%x", chip_id.whoami);
		return -EIO;
	}
