/* ST Microelectronics IIS3DWB10IS accelerometer sensor
 *
...
LOG_MODULE_REGISTER(IIS3DWB10IS_DECODER, CONFIG_SENSOR_LOG_LEVEL);

#ifdef CONFIG_IIS3DWB10IS_STREAM
static const uint32_t accel_period_ns[] = {
	[IIS3DWB10IS_DT_XL_NOT_BATCHED] = UINT32_C(0),
	[IIS3DWB10IS_DT_XL_BATCHED_AT_26k7Hz] = UINT32_C(1000000000) / 26700,
};

#if defined(CONFIG_IIS3DWB10IS_ENABLE_TEMP)
static const uint32_t temp_period_ns[] = {
	[IIS3DWB10IS_DT_TEMP_NOT_BATCHED] = UINT32_C(0),
	[IIS3DWB10IS_DT_TEMP_BATCHED_AT_104Hz] = UINT32_C(1000000000) / 104,
};
#endif
#endif /* CONFIG_IIS3DWB10IS_STREAM */
...
static const int8_t accel_range[] = {
	[IIS3DWB10IS_DT_FS_5g] = 6,
	[IIS3DWB10IS_DT_FS_100g] = 10,
	[IIS3DWB10IS_DT_FS_200g] = 11,
};
...
static const int32_t accel_scaler[] = {
	[IIS3DWB10IS_DT_FS_2G] = SENSOR_SCALE_UG_TO_UMS2(GAIN_UNIT),
	[IIS3DWB10IS_DT_FS_4G] = SENSOR_SCALE_UG_TO_UMS2(2 * GAIN_UNIT),
	[IIS3DWB10IS_DT_FS_8G] = SENSOR_SCALE_UG_TO_UMS2(4 * GAIN_UNIT),
	[IIS3DWB10IS_DT_FS_16G] = SENSOR_SCALE_UG_TO_UMS2(8 * GAIN_UNIT),
};
...
static int iis3dwb10is_decoder_get_frame_count(const uint8_t *buffer, struct sensor_chan_spec chan_spec,
					   uint16_t *frame_count)
{
	const struct iis3dwb10is_fifo_data *data = (const struct iis3dwb10is_fifo_data *)buffer;
	const struct iis3dwb10is_rtio_data *rdata = (const struct iis3dwb10is_rtio_data *)buffer;
	const struct iis3dwb10is_decoder_header *header = &data->header;
...
#ifdef CONFIG_IIS3DWB10IS_STREAM
	const struct iis3dwb10is_fifo_data *edata = (const struct iis3dwb10is_fifo_data *)buffer;
	...
	buffer += sizeof(struct iis3dwb10is_fifo_data);
	buffer_end = buffer + IIS3DWB10IS_FIFO_SIZE(edata->fifo_count);
	...
	while (buffer < buffer_end) {
		...
		switch (fifo_tag) {
		case IIS3DWB10IS_XL_TAG:
			tot_accel_fifo_words++;
			break;
#if defined(CONFIG_IIS3DWB10IS_ENABLE_TEMP)
		case IIS3DWB10IS_TEMPERATURE_TAG:
			tot_temp_fifo_words++;
			break;
#endif
		default:
		...
		buffer += IIS3DWB10IS_FIFO_ITEM_LEN;
	}
...
#ifdef CONFIG_IIS3DWB10IS_STREAM
static int iis3dwb10is_decode_fifo(const uint8_t *buffer, struct sensor_chan_spec chan_spec,
			       uint32_t *fit, uint16_t max_count, void *data_out)
{
	const struct iis3dwb10is_fifo_data *edata = (const struct iis3dwb10is_fifo_data *)buffer;
	const uint8_t *buffer_end;
	const struct iis3dwb10is_decoder_header *header = &edata->header;
	int count = 0;
	uint8_t fifo_tag;
	uint16_t xl_count = 0;
#if defined(CONFIG_IIS3DWB10IS_ENABLE_TEMP)
	...
	/* count total FIFO word for each tag */
	ret = iis3dwb10is_decoder_get_frame_count(buffer, chan_spec, &tot_fifo_samples);
	if (ret < 0) {
		return 0;
	}
	...
	while (count < max_count && buffer < buffer_end) {
		...
		skip_frame = 0;
		frame_end += IIS3DWB10IS_FIFO_ITEM_LEN;
...
static int iis3dwb10is_decode_one_shot(const uint8_t *buffer, struct sensor_chan_spec chan_spec,
				   uint32_t *fit, uint16_t max_count, void *data_out)
{
	const struct iis3dwb10is_rtio_data *edata = (const struct iis3dwb10is_rtio_data *)buffer;
	const struct iis3dwb10is_decoder_header *header = &edata->header;
...
static int iis3dwb10is_decoder_decode(const uint8_t *buffer, struct sensor_chan_spec chan_spec,
				  uint32_t *fit, uint16_t max_count, void *data_out)
{
#ifdef CONFIG_IIS3DWB10IS_STREAM
	const struct iis3dwb10is_decoder_header *header = (const struct iis3dwb10is_decoder_header *)buffer;

	if (header->is_fifo) {
		return iis3dwb10is_decode_fifo(buffer, chan_spec, fit, max_count, data_out);
	}
#endif

	return iis3dwb10is_decode_one_shot(buffer, chan_spec, fit, max_count, data_out);
}

static int iis3dwb10is_decoder_get_size_info(struct sensor_chan_spec chan_spec, size_t *base_size,
					 size_t *frame_size)
{
	switch (chan_spec.chan_type) {
	case SENSOR_CHAN_ACCEL_X:
...
static bool iis3dwb10is_decoder_has_trigger(const uint8_t *buffer, enum sensor_trigger_type trigger)
{
#ifdef CONFIG_IIS3DWB10IS_STREAM
	const struct iis3dwb10is_decoder_header *header = (const struct iis3dwb10is_decoder_header *)buffer;
...
SENSOR_DECODER_API_DT_DEFINE() = {
	.get_frame_count = iis3dwb10is_decoder_get_frame_count,
	.get_size_info = iis3dwb10is_decoder_get_size_info,
	.decode = iis3dwb10is_decoder_decode,
	.has_trigger = iis3dwb10is_decoder_has_trigger,
};

int iis3dwb10is_get_decoder(const struct device *dev, const struct sensor_decoder_api **decoder)
{
	ARG_UNUSED(dev);
