/* ST Microelectronics LSM6DSVXXX 6-axis IMU sensor driver
 *
...
static int lsm6dsvxxx_shub_write_target_reg(const struct device *dev,
					 uint8_t trgt_addr, uint8_t trgt_reg,
					 uint8_t *value, uint16_t len);
static int lsm6dsvxxx_shub_read_target_reg(const struct device *dev,
					uint8_t trgt_addr, uint8_t trgt_reg,
					uint8_t *value, uint16_t len);
#if 0
static void lsm6dsvxxx_shub_enable(const struct device *dev, uint8_t enable);


/* ST HAL skips this register, only supports it via the slower lsm6dsvxxx_sh_status_get() */
static int32_t lsm6dsvxxx_sh_status_mainpage_get(stmdev_ctx_t *ctx,
						 lsm6dsvxxx_status_master_t *val)
{
	return lsm6dsvxxx_read_reg(ctx, LSM6DSVXXX_STATUS_MASTER_MAINPAGE, (uint8_t *)val, 1);
}
#endif
 ...
 */
#ifdef CONFIG_LSM6DSVXXX_EXT_LIS2MDL
...
static int lsm6dsvxxx_lis2mdl_init(const struct device *dev, uint8_t i2c_addr)
{
	...
	lsm6dsvxxx_shub_write_target_reg(dev, i2c_addr,
...
static int lsm6dsvxxx_lis2mdl_odr_set(const struct device *dev,
				      uint8_t i2c_addr, uint16_t freq)
{
	...
	config->chip_api->shub_enable(dev, 1);
	return 0;
}

static int lsm6dsvxxx_lis2mdl_conf(const struct device *dev, uint8_t i2c_addr,
				   ...
				   const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		return lsm6dsvxxx_lis2mdl_odr_set(dev, i2c_addr, val->val1);
	default:
...
#endif /* CONFIG_LSM6DSVXXX_EXT_LIS2MDL */

/*
 * HTS221 humidity device specific part
 */
#ifdef CONFIG_LSM6DSVXXX_EXT_HTS221
...
static int lsm6dsvxxx_hts221_read_conv_data(const struct device *dev,
					    uint8_t i2c_addr)
{
	struct lsm6dsvxxx_data *data = dev->data;
	uint8_t buf[16], i;
	...
	for (i = 0; i < sizeof(buf); i += 7) {
		...
		if (lsm6dsvxxx_shub_read_target_reg(dev, i2c_addr,
...
static int lsm6dsvxxx_hts221_init(const struct device *dev, uint8_t i2c_addr)
{
	uint8_t hum_cfg;
	...
	return lsm6dsvxxx_hts221_read_conv_data(dev, i2c_addr);
}

static const uint16_t hts221_map[] = {0, 1, 7, 12};

static int lsm6dsvxxx_hts221_odr_set(const struct device *dev,
				     uint8_t i2c_addr, uint16_t freq)
{
	const struct lsm6dsvxxx_config *config = dev->config;
	uint8_t odr, cfg;
...
static int lsm6dsvxxx_hts221_conf(const struct device *dev, uint8_t i2c_addr,
				  enum sensor_channel chan,
				  enum sensor_attribute attr,
				  const struct sensor_value *val)
{
	switch (attr) {
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		return lsm6dsvxxx_hts221_odr_set(dev, i2c_addr, val->val1);
	default:
...
#endif /* CONFIG_LSM6DSVXXX_EXT_HTS221 */

/*
 * LPS22HB baro/temp device specific part
 */
#ifdef CONFIG_LSM6DSVXXX_EXT_LPS22HB
...
static int lsm6dsvxxx_lps22hb_init(const struct device *dev, uint8_t i2c_addr)
{
	uint8_t baro_cfg[2];
...
#endif /* CONFIG_LSM6DSVXXX_EXT_LPS22HB */

/*
 * LPS22HH baro/temp device specific part
 */
#ifdef CONFIG_LSM6DSVXXX_EXT_LPS22HH
...
static int lsm6dsvxxx_lps22hh_init(const struct device *dev, uint8_t i2c_addr)
{
	uint8_t baro_cfg[2];
...
static int lsm6dsvxxx_lps22hh_odr_set(const struct device *dev,
				      uint8_t i2c_addr, uint16_t freq)
{
	uint8_t odr, cfg;
	...
	lsm6dsvxxx_shub_enable(dev, 1);
	return 0;
}

static int lsm6dsvxxx_lps22hh_conf(const struct device *dev, uint8_t i2c_addr,
				   enum sensor_channel chan,
				   enum sensor_attribute attr,
				   const struct sensor_value *val)
{
	switch (attr) {
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		return lsm6dsvxxx_lps22hh_odr_set(dev, i2c_addr, val->val1);
	default:
...
#endif /* CONFIG_LSM6DSVXXX_EXT_LPS22HH */

/*
 * LPS22DF baro/temp device specific part
 */
#ifdef CONFIG_LSM6DSVXXX_EXT_LPS22DF
...
static int lsm6dsvxxx_lps22df_init(const struct device *dev, uint8_t i2c_addr)
{
	uint8_t baro_cfg[2];
...
static int lsm6dsvxxx_lps22df_odr_set(const struct device *dev,
				      uint8_t i2c_addr, uint16_t freq)
{
	const struct lsm6dsvxxx_config *config = dev->config;
	uint8_t odr, cfg;
...
static int lsm6dsvxxx_lps22df_conf(const struct device *dev, uint8_t i2c_addr,
				    enum sensor_channel chan,
				    enum sensor_attribute attr,
				    const struct sensor_value *val)
{
	switch (attr) {
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		return lsm6dsvxxx_lps22df_odr_set(dev, i2c_addr, val->val1);
	default:
...
#endif /* CONFIG_LSM6DSVXXX_EXT_LPS22DF */

/* List of supported external sensors */
static struct lsm6dsvxxx_shub_slist {
...
#ifdef CONFIG_LSM6DSVXXX_EXT_LIS2MDL
	{
		...
		.out_data_addr  = 0x68,
		.out_data_len   = 0x06,
		.dev_init	= (lsm6dsvxxx_lis2mdl_init),
		.dev_conf	= (lsm6dsvxxx_lis2mdl_conf),
	},
#endif /* CONFIG_LSM6DSVXXX_EXT_LIS2MDL */
...
#ifdef CONFIG_LSM6DSVXXX_EXT_HTS221
	{
		...
		.out_data_len   = 0x02,
		.dev_init	= (lsm6dsvxxx_hts221_init),
		.dev_conf	= (lsm6dsvxxx_hts221_conf),
	},
#endif /* CONFIG_LSM6DSVXXX_EXT_HTS221 */
...
#ifdef CONFIG_LSM6DSVXXX_EXT_LPS22HB
	{
		...
		.out_data_addr  = 0x28,
		.out_data_len   = 0x05,
		.dev_init	= (lsm6dsvxxx_lps22hb_init),
	},
#endif /* CONFIG_LSM6DSVXXX_EXT_LPS22HB */
...
#ifdef CONFIG_LSM6DSVXXX_EXT_LPS22HH
	{
		...
		.out_data_addr  = 0x28,
		.out_data_len   = 0x05,
		.dev_init	= (lsm6dsvxxx_lps22hh_init),
		.dev_conf	= (lsm6dsvxxx_lps22hh_conf),
	},
#endif /* CONFIG_LSM6DSVXXX_EXT_LPS22HH */
...
#ifdef CONFIG_LSM6DSVXXX_EXT_LPS22DF
	{
		...
		.out_data_addr  = 0x28,
		.out_data_len   = 0x05,
		.dev_init	= (lsm6dsvxxx_lps22df_init),
		.dev_conf	= (lsm6dsvxxx_lps22df_conf),
	},
#endif /* CONFIG_LSM6DSVXXX_EXT_LPS22DF */
};

#if 0
static int lsm6dsvxxx_shub_wait_completed(stmdev_ctx_t *ctx)
{
	lsm6dsvxxx_status_master_t status;
	...
	do {
		...
		k_msleep(1);
		lsm6dsvxxx_sh_status_mainpage_get(ctx, &status);
...
static void lsm6dsvxxx_shub_enable(const struct device *dev, uint8_t enable)
{
	...
	if (!data->accel_freq) {
		...
		if (lsm6dsvxxx_xl_data_rate_set(ctx, odr) < 0) {
	...
	if (lsm6dsvxxx_sh_master_set(ctx, enable) < 0) {
		LOG_DBG("shub: failed to set master on");
		lsm6dsvxxx_mem_bank_set(ctx, LSM6DSVXXX_MAIN_MEM_BANK);
		return;
	}
...
static int lsm6dsvxxx_shub_check_slv0_nack(stmdev_ctx_t *ctx)
{
	lsm6dsvxxx_all_sources_t status;

	if (lsm6dsvxxx_all_sources_get(ctx, &status) < 0) {
					   ...
					   uint8_t *value, uint16_t len)
{
	const struct lsm6dsvxxx_config *cfg = dev->config;
	stmdev_ctx_t *ctx = (stmdev_ctx_t *)&cfg->ctx;
	lsm6dsvxxx_sh_cfg_read_t trgt_cfg;

	trgt_cfg.slv_add = trgt_addr;
	trgt_cfg.slv_subadd = trgt_reg;
	trgt_cfg.slv_len = len;

	lsm6dsvxxx_sh_slv_cfg_read(ctx, 0, &trgt_cfg);

	/* turn SH on, wait for shub i2c read to finish */
	cfg->chip_api->shub_enable(dev, 1);
	lsm6dsvxxx_shub_wait_completed(ctx);

	/* read data from external target */
	if (lsm6dsvxxx_sh_read_data_raw_get(ctx, value, len) < 0) {
		LOG_DBG("shub: error reading sensor data");
		return -EIO;
	}

	if (lsm6dsvxxx_shub_check_slv0_nack(ctx) < 0) {
		cfg->chip_api->shub_enable(dev, 1);
		return -EIO;
	}
					    ...
					    uint8_t *value, uint16_t len)
{
	...
	while (cnt < len) {
		...
		cfg->chip_api->shub_cfg_write(dev, trgt_addr, trgt_reg + cnt, value[cnt]);
	...
	cfg->chip_api->shub_cfg_write(dev, &trgt_cfg);
 ...
 */
static int lsm6dsvxxx_shub_set_data_channel(const struct device *dev)
{
	struct lsm6dsvxxx_data *data = dev->data;
	const struct lsm6dsvxxx_config *cfg = dev->config;
	stmdev_ctx_t *ctx = (stmdev_ctx_t *)&cfg->ctx;
	uint8_t n;
	struct lsm6dsvxxx_shub_slist *sp;
	lsm6dsvxxx_sh_cfg_read_t trgt_cfg;

	/* Configure shub data channels to access external targets */
	for (n = 0; n < data->num_ext_dev; n++) {
		sp = &lsm6dsvxxx_shub_slist[data->shub_ext[n]];

		trgt_cfg.slv_add = sp->ext_i2c_addr;
		trgt_cfg.slv_subadd = sp->out_data_addr;
		trgt_cfg.slv_len = sp->out_data_len;

		if (lsm6dsvxxx_sh_slv_cfg_read(ctx, n + 1, &trgt_cfg) < 0) {
	...
	lsm6dsvxxx_sh_slave_connected_t aux = LSM6DSVXXX_SLV_0_1_2;

	if (lsm6dsvxxx_sh_slave_connected_set(ctx, aux) < 0) {
...
int lsm6dsvxxx_shub_get_idx(const struct device *dev, enum sensor_channel type)
{
	uint8_t n;
	struct lsm6dsvxxx_data *data = dev->data;
	struct lsm6dsvxxx_shub_slist *sp;
...
#if 0
int lsm6dsvxxx_shub_fetch_external_devs(const struct device *dev)
{
	uint8_t n;
	const struct lsm6dsvxxx_config *cfg = dev->config;
	stmdev_ctx_t *ctx = (stmdev_ctx_t *)&cfg->ctx;
	...
	/* read data from external target */
	if (lsm6dsvxxx_mem_bank_set(ctx, LSM6DSVXXX_SENSOR_HUB_MEM_BANK) < 0) {
	...
	for (n = 0; n < data->num_ext_dev; n++) {
		...
		if (lsm6dsvxxx_read_reg(ctx, sp->sh_out_reg,
				     data->ext_data[n], sp->out_data_len) < 0) {
			LOG_DBG("shub: failed to read sample");
			(void) lsm6dsvxxx_mem_bank_set(ctx, LSM6DSVXXX_MAIN_MEM_BANK);
			return -EIO;
		}
	}

	return lsm6dsvxxx_mem_bank_set(ctx, LSM6DSVXXX_MAIN_MEM_BANK);
}
#endif

int lsm6dsvxxx_shub_config(const struct device *dev, enum sensor_channel chan,
			enum sensor_attribute attr,
			const struct sensor_value *val)
{
	struct lsm6dsvxxx_data *data = dev->data;
	struct lsm6dsvxxx_shub_slist *sp = NULL;
	uint8_t n;
...
int lsm6dsvxxx_shub_init(const struct device *dev)
{
	struct lsm6dsvxxx_data *data = dev->data;
	const struct lsm6dsvxxx_config *cfg = dev->config;
	stmdev_ctx_t *ctx = (stmdev_ctx_t *)&cfg->ctx;
	...
	/*
	 * This must be set or lsm6dsvxxx_shub_write_target_reg() will
	 * repeatedly write the same regi
	 */
	if (lsm6dsvxxx_sh_write_mode_set(ctx, LSM6DSVXXX_ONLY_FIRST_CYCLE) < 0) {
		LOG_DBG("shub: error setting write once");
		return -EIO;
	}

	for (n = 0; n < ARRAY_SIZE(lsm6dsvxxx_shub_slist); n++) {
		if (data->num_ext_dev >= LSM6DSVXXX_SHUB_MAX_NUM_TARGETS) {
			break;
		}

		chip_id = 0;
		sp = &lsm6dsvxxx_shub_slist[n];
		...
		for (i = 0U; i < ARRAY_SIZE(sp->i2c_addr); i++) {
			if (lsm6dsvxxx_shub_read_target_reg(dev,
							    sp->i2c_addr[i],
							    sp->wai_addr,
							    &chip_id, 1) < 0) {
	...
	for (n = 0, regn = 0; n < data->num_ext_dev; n++) {
		sp = &lsm6dsvxxx_shub_slist[data->shub_ext[n]];
		sp->sh_out_reg = LSM6DSVXXX_SENSOR_HUB_1 + regn;
		regn += sp->out_data_len;
		sp->dev_init(dev, sp->ext_i2c_addr);
	}

	lsm6dsvxxx_shub_set_data_channel(dev);
