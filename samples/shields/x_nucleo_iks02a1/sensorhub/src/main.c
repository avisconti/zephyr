...
void main(void)
{
	...
	if (iis2iclx_0) {
		trig.type = SENSOR_TRIG_DATA_READY;
		trig.chan = SENSOR_CHAN_ACCEL_XYZ;
		sensor_trigger_set(iis2iclx_0, &trig, iis2iclx_acc_0_trig_handler);
	...
	if (iis2iclx_1) {
		trig.type = SENSOR_TRIG_DATA_READY;
		trig.chan = SENSOR_CHAN_ACCEL_XYZ;
		sensor_trigger_set(iis2iclx_1, &trig, iis2iclx_acc_1_trig_handler);
	}
	...
	while (1) {
		...
		if (iis2iclx_0 && sensor_sample_fetch(iis2iclx_0) < 0) {
			printf("IIS2ICLX 0 Sensor sample update error\n");
			return;
		}

		if (iis2iclx_1 && sensor_sample_fetch(iis2iclx_1) < 0) {
